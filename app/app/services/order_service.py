...
def get_all_orders(db: Session):
    """Retorna todos os pedidos do banco de dados, dos mais recentes para os mais antigos."""
    return db.query(Order).order_by(Order.orderDate.desc()).all()

# --- NOVA FUNÇÃO ---
def get_orders_by_cinema(db: Session, cinema_id: int):
    """Retorna todos os pedidos de um cinema específico."""
    return db.query(Order).filter(Order.cinema_id == cinema_id).order_by(Order.orderDate.desc()).all()
# --------------------
